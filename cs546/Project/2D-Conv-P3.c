...
void sync_tasks(complex a[512*512], complex b[512*512], complex A[512*512], complex B[512*512], int p, int my_rank) {
	
	int workload = 512/(p/2);

	/* Send all a's and b's to the root process */
	if(my_rank < p/2) {
		MPI_Gather(a, workload*512, mpi_complex, 
				   A, workload*512, mpi_complex,
				   0, comm1);
	} else {
